...
from src.papers import (
    ...
    flush_papers,
    get_paper_info,
    get_papers_by_project_filter,
    get_papers_by_topic_filter,
    get_all_papers_list,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Download and organize research papers"
    )
    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    # Add command (top-level)
    add_parser = subparsers.add_parser(
        "add", help="Add paper (metadata + download + Notion)"
    )
    add_parser.add_argument("url", help="URL of the paper to add")

    # Sync command (top-level)
    subparsers.add_parser(
        "sync", help="Sync papers from config to downloads and Notion"
    )
    ...
    flush_parser = subparsers.add_parser(
        "flush", help="Flush papers from download_dir to archive_dir"
    )
    flush_parser.add_argument("names", nargs="+", help="Paper codenames to flush")

    # Get command (top-level)
    get_parser = subparsers.add_parser(
        "get", help="Query paper information from Notion"
    )
    ...
    download_parser = subparsers.add_parser(
        "download", help="Download a research paper"
    )
    download_parser.add_argument(
        "--url", required=True, help="URL of the paper to download"
    )
    download_parser.add_argument(
        "--file",
        "-f",
        help="Custom filename for the downloaded paper (without .pdf extension)",
    )

    # Notion command
    notion_parser = subparsers.add_parser("notion", help="Notion integration commands")
    notion_parser.add_argument(
        "--setup", action="store_true", help="Set up Notion integration"
    )
    notion_parser.add_argument(
        "--add", action="store_true", help="Add a paper to Notion database"
    )
